...
public class KView extends PoolableImpl implements TouchResponder {
	...
	// Resize mode
	public static final byte ResizeModeStretchHeight = 32;
	public static final byte ResizeModeStretchWidth = 16;
	public static final byte ResizeModeKeepLeftMargin = 8;
	public static final byte ResizeModeKeepRightMargin = 4;
	public static final byte ResizeModeKeepTopMargin = 2;
	public static final byte ResizeModeKeepBottomMargin = 1;
	public static final byte ResizeModeNone = 0;
	public static final byte ResizeModeAll = (byte)0xFF;
	
	final private static Color tmpColor = new Color();
	
	final private SnapshotArray<KView> views;
	...
	final private Rectangle oldFrame;
	
	private byte resizeModeMask;
	private SnapshotArray<Animation> animations;
	private Drawable background;
	private KView parentView;
	...
	private boolean shouldLayoutSubviews;
	private boolean autoresizeViews;
	...
	public KView() {
		this.views = new SnapshotArray<KView>(true, 4, KView.class);
		this.screenFrame = new Rectangle();
		this.frame = new Rectangle();
		this.renderingFrame = new Rectangle();
		this.oldFrame = new Rectangle();
	...
	final private void restoreTransformToContext(DrawingContext context) {
	 ...
	 protected void drawSubviews(DrawingContext context, float currentScreenX, float currentScreenY, float currentAlpha) {
		KView[] views = this.views.begin();
		for (int i = 0, length = this.views.size; i < length; i++) {
			final KView view = views[i];
...
//		}
	...
	public void update(float deltaTime) {
		...
		if (this.shouldLayoutSubviews) {
			this.shouldLayoutSubviews = false;
			this.layoutViews();
		}
		...
		KView[] subViews = this.views.begin();
	...
	public <T extends KView> void addViews(Array<T> views) {
		for (int i = 0, length = views.size; i < length; i++) {
			this.addView(views.get(i));
		}
	}
	
	public void addViews(KView ... views) {
		for (int i = 0, length = views.length; i < length; i++) {
			this.addView(views[i]);
		}
	}
	
	public void addView(KView view) {
		if (view == null) {
	...
	public boolean removeView(KView view) {
		if (view == null) {
			throw new IllegalArgumentException("view may not be null");
		}
	...
	public Vector2 getPositionForViewCoordinate(KView otherView, Vector2 outputVector) {
		outputVector.x = 0;
		outputVector.y = 0;

		KView currentView = this;
	...
	public TouchResponder getBestTouchResponderForLocation(float x, float y) {
		...
		final KView[] views = this.views.items;
		for (int i = this.views.size - 1; bestResponder == null &&i >= 0; i--) {
			final KView currentView = views[i];
	...
	public void layoutViews() {
		if (this.autoresizeViews) {
			byte resizeMask = this.resizeModeMask;
			float xRatio = this.oldFrame.width / this.frame.width;
			float yRatio = this.oldFrame.height / this.frame.height;
			
			System.out.println(xRatio + "/" + yRatio);
			KView[] views = this.views.items;
			for (int i = 0, length = this.views.size; i < length; i++) {
				KView view = views[i];
				Rectangle viewFrame = view.frame;
				
				if ((resizeMask & ResizeModeKeepBottomMargin) == 0) {
					viewFrame.y *= yRatio;
				}
				if ((resizeMask & ResizeModeKeepLeftMargin) == 0) {
					viewFrame.x *= xRatio;
				}
				if ((resizeMask & ResizeModeKeepTopMargin) != 0) {
//					viewFrame.height *= yRatio;
				}
				if ((resizeMask & ResizeModeKeepRightMargin) != 0) {
					
				}
				if ((resizeMask & ResizeModeStretchWidth) != 0) {
					
				}
				if ((resizeMask & ResizeModeStretchHeight) != 0) {
	...
	final private void didChangeFrame() {
		...
		this.shouldLayoutSubviews = true;
	}
	 ...
	 * If you want to change the view frame you need to call setFrame()
	...
	public byte getResizeModeMask() {
		return resizeModeMask;
	}

	public void setResizeModeMask(byte resizeModeMask) {
		this.resizeModeMask = resizeModeMask;
	}

	public boolean isAutoresizeViews() {
		return autoresizeViews;
	}

	public void setAutoresizeViews(boolean autoresizeViews) {
		this.autoresizeViews = autoresizeViews;
	}
}
